...
main =
  runTest do
    ...
    suite "Exercise Group - Applicative and Effects" do
      ...
      suite "Exercise - combineMaybeAdo" do
        suite "Array Int" do
          test "Just" do
            Assert.equal ([ Just 1, Just 2, Just 3 ])
              $ combineMaybeAdo (Just $ [ 1, 2, 3 ])
          test "Nothing" do
            Assert.equal ([ Nothing ])
              $ combineMaybeAdo (Nothing ∷ Maybe (Array Int))
        suite "List Char" do
          test "Just" do
            Assert.equal (Just 'a' : Just 'b' : Just 'c' : Nil)
              $ combineMaybeAdo (Just $ 'a' : 'b' : 'c' : Nil)
          test "Nothing" do
            Assert.equal (Nothing : Nil)
              $ combineMaybeAdo (Nothing ∷ Maybe (List Char))
